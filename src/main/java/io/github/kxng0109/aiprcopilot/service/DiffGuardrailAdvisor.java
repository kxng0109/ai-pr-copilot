 ...
 * size cap, instruction-override denylist, then {@link SecretScanService}.
 * BLOCK verdicts hard-fail with {@link BlockedDiffException} (no LLM cost);
 * REDACT verdicts rewrite user messages with the redacted text and continue.
 */
@Component
@Slf4j
...
public class DiffGuardrailAdvisor implements CallAdvisor, StreamAdvisor {
    ...
    private ChatClientRequest guard(ChatClientRequest request) {
        ...
        String haystack = contents == null ? "" : contents.toLowerCase(Locale.ROOT);
