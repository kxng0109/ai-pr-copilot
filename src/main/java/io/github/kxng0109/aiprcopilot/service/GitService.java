...
public class GitService {
    ...
    private String runCommand(String... command) {
        ...
        try {
            ...
            Thread outputThread = new Thread(() -> {
                try {
                    outputBuilder.append(new String(finalProcess.getInputStream().readAllBytes(), StandardCharsets.UTF_8));
                } catch (IOException e) {
    ...
    /**
     * Non-zero git exit with the code preserved, so callers can distinguish
     * meaningful codes (e.g. {@code diff --quiet} exit 1 = differences exist).
     */
