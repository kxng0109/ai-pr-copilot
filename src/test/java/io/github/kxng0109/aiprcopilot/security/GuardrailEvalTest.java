 ...
 * so it cannot anchor a regression baseline. Every assertion here is a
 * pure function of the scanner verdict and the guardrail decision.
 *
 * <p>The harness measures and REPORTS. It asserts only that the current
 * numbers do not regress below a pinned baseline, so a real gap stays
 * visible instead of being papered over or hidden.
 *
 * <p>Current measurements (2026-09-10):
 *   secret-scan recall = 6/7
 *   obfuscation recall = 0/3 -- base64/rot13/zero-width variants of a
 *       GitHub token bypass the scanner. Documented gap, not silent.
 *   injection denylist = 3/5
 *
 * <p>Residual-risk statement (required, not optional): per OWASP LLM01:2026,
 * NIST and NCSC, deterministic prevention of prompt injection is currently
 * impossible. Static attack success can approach 0% while adaptive success
 * exceeds 90% (Nasr et al. 2025). Re-run whenever the corpus or defense
 * changes.
 */
class GuardrailEvalTest {
    ...
    private int countDetected(List<String> samples) {
        ...
        for (String s : samples) {
            if (scanner.scan(s).verdict() != SecretScanService.Verdict.CLEAN) detected++;
        }
        return detected;
    }
    ...
    private int countInjectionStopped(List<String> payloads) throws Exception {
        ...
        for (String p : payloads) {
            ...
            if (denylist.stream().anyMatch(pat -> pat.matcher(hay).find())) stopped++;
        }
        return stopped;
    }
    ...
    private void assertNotBelowBaseline(String name, int current, int total) {
        ...
        System.out.printf("%s = %d/%d (baseline %d/%d)%n",
