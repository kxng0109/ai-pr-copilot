 ...
 * environment variable {@code AI_PROVIDER_KEY} is set, so a credential
 * less CI run never touches a network and never leaks a key into a
 * report. Keys arrive via env vars, never committed.
 *
 * <p>Each test makes exactly one real API call and asserts only on
 * response shape (status, non-null body, leveled-risk contract) — never
 * on model content, which is non-deterministic.
 */
